# Simple Dune Awakening Manager GUI.
...
$ManagerName = 'Simple Dune Awakening Manager'
$ManagerTitle = "$ManagerName by Ace"
$AutomationScript = Join-Path $PSScriptRoot 'Simple-Dune-Awakening-Manager-Automation.ps1'
...
[System.Windows.Forms.Application]::add_ThreadException({
    ...
    [System.Windows.Forms.MessageBox]::Show($eventArgs.Exception.Message, "$ManagerName error") | Out-Null
})
...
function Start-Worker {
    ...
    if ($script:CurrentProcess -and -not $script:CurrentProcess.HasExited) {
        if (-not $QuietIfBusy) {
            [System.Windows.Forms.MessageBox]::Show('A task is already running.', $ManagerName) | Out-Null
        }
        return $false
    }
    ...
    if (-not (Test-Path $AutomationScript)) {
        if (-not $QuietIfBusy) {
            [System.Windows.Forms.MessageBox]::Show("Automation script missing: $AutomationScript", $ManagerName) | Out-Null
        }
        return $false
    }
    ...
    try {
        ...
        Set-Busy $false
        if (-not $QuietIfBusy) {
            [System.Windows.Forms.MessageBox]::Show($_.Exception.Message, "$ManagerName action failed") | Out-Null
        }
        return $false
    }
}
...
function Refresh-Status {
    ...
    $script:StatusSummary.Text = if (-not $admin) {
        'Relaunch through Start-Simple-Dune-Awakening-Manager.bat for full automation.'
    } elseif (-not $server) {
...
function Start-InitialSetupFromUi {
    try {
        if (-not $ServerRoot -or -not (Test-Path -LiteralPath $ServerRoot)) {
            throw 'Could not find the Dune Awakening Self-Hosted Server Steam package. Install it through Steam Tools or set DUNE_SERVER_ROOT before launching Simple Dune Awakening Manager.'
        }
...
function Open-VmShell {
    $ip = Get-DuneVmIp
    if (-not $ip) {
        [System.Windows.Forms.MessageBox]::Show('VM is not running or has no IP address.', $ManagerName) | Out-Null
        return
    }
    if (-not (Test-Path $SshKey)) {
        [System.Windows.Forms.MessageBox]::Show("SSH key is missing: $SshKey", $ManagerName) | Out-Null
        return
    }
...
function Start-RestoreBackupFromUi {
    try {
        ...
        $dialog.Title = 'Select Simple Dune Awakening Manager backup archive'
        $dialog.InitialDirectory = $BackupsDir
        $dialog.Filter = 'Simple Dune Awakening Manager backup (*.tar.gz)|*.tar.gz|All files (*.*)|*.*'
...
function Show-LicenseMessage {
    $message = @'
Simple Dune Awakening Manager usage note
...
Ace
'@
    [System.Windows.Forms.MessageBox]::Show($message, "$ManagerName License / Use") | Out-Null
}
...
$openServerButton.Add_Click({
    if ($ServerRoot -and (Test-Path -LiteralPath $ServerRoot)) {
        ...
        [System.Windows.Forms.MessageBox]::Show('Steam server package not found. Install it through Steam Tools or set DUNE_SERVER_ROOT before launching Simple Dune Awakening Manager.', $ManagerName) | Out-Null
    }
})
...
$script:SetupStateGroup.SetBounds(16, 12, 410, 100)
$setupTab.Controls.Add($script:SetupStateGroup)
$script:SetupStateLabel = New-Label 'Checking install state...' 14 22 382 64
$script:SetupStateLabel.ForeColor = [Drawing.Color]::DarkBlue
...
$script:CompatibilityWarningsCheck.SetBounds(16, 438, 340, 24)
...
$script:SwapCheck.SetBounds(16, 468, 260, 24)
...
$script:StartAfterSetupCheck.SetBounds(16, 498, 240, 24)
$setupTab.Controls.Add($script:StartAfterSetupCheck)

$script:SetupStartButton = New-Button 'Run Automated Setup' 16 544 390 42
...
$signatureLabel = New-Label $ManagerTitle 604 9 190
$signatureLabel.AutoSize = $false
$signatureLabel.TextAlign = [Drawing.ContentAlignment]::MiddleRight
$bottomPanel.Controls.Add($signatureLabel)

function Update-BottomPanelLayout {
    $rightEdge = $bottomPanel.ClientSize.Width
    if ($rightEdge -le 0) { return }

    $signatureLeft = [Math]::Max(0, $rightEdge - 200)
    $signatureLabel.SetBounds($signatureLeft, 9, 190, 22)

    $cancelLeft = [Math]::Max(0, $signatureLeft - 104)
    $cancelTaskButton.SetBounds($cancelLeft, 5, 92, 26)

    $progressLeft = 240
    $progressWidth = [Math]::Max(120, $cancelLeft - $progressLeft - 14)
    $script:Progress.SetBounds($progressLeft, 8, $progressWidth, 18)
}

$script:Progress.Anchor = 'Top,Left'
$cancelTaskButton.Anchor = 'Top,Left'
$bottomPanel.Add_Resize({ Update-BottomPanelLayout })
...
$script:Form.Add_Shown({
    try {
        $setupTab.AutoScrollPosition = [Drawing.Point]::new(0, 0)
    } catch {}
})
...
Add-LogLine "$ManagerName UI ready."
